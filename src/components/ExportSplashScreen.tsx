...
import { FileArchive, Database, FolderOpen, CheckCircle, Loader2 } from 'lucide-react';

interface ExportSplashScreenProps {
  isVisible: boolean;
  progress: {
    step: string;
    currentSite: string;
...
const ExportSplashScreen: React.FC<ExportSplashScreenProps> = ({ isVisible, progress }) => {
  ...
  const steps = [
    ...
    { id: 'creating-structure', label: 'Creating export structure', icon: FolderOpen },
    { id: 'generating-zip', label: 'Generating ZIP file', icon: FileArchive },
    { id: 'complete', label: 'Export ready', icon: CheckCircle }
  ];
      ...
      <div className="bg-white rounded-lg shadow-2xl max-w-md w-full mx-4 p-6">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-600 text-sm">
            Please wait while we prepare your export ZIP file
          </p>
        </div>
        ...
        <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-xs text-yellow-800">
            <strong>Please wait:</strong> Do not close this window or navigate away while the export is being prepared.
          </p>
        </div>
      </div>
    </div>
...
export default ExportSplashScreen;